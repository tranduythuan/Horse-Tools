 ...
 * The snippets screen.
 *
 * Shared verbatim by the Shortcode settings page and the Content → Shortcode
 * tab, which used to hold byte-identical copies of it. One screen, one file:
 * a change to the list, the editor or the AJAX contract now happens once.
 *
 * @package Horse Tools
 */
...
?>
			<div class="ht-card ht-snip" data-nonce="<?php echo esc_attr( wp_create_nonce( 'horsetools_snip' ) ); ?>" data-ajax="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>">
			  <h3><i class="ti ti-file-code"></i> <?php _e('Custom shortcodes (snippets)', 'horse-tools') ?></h3>
				...
				<div class="ht-snip-pager" id="ht-snip-pager" hidden>
					<button type="button" class="ht-snip-page" data-step="-1">&larr; <?php esc_html_e( 'Previous', 'horse-tools' ); ?></button>
					<span id="ht-snip-pagelbl"></span>
					<button type="button" class="ht-snip-page" data-step="1"><?php esc_html_e( 'Next', 'horse-tools' ); ?> &rarr;</button>
				</div>
			</div>
			...
			.ht-snip-pager{display:flex;align-items:center;gap:12px;justify-content:center;margin:8px 0 2px;font-size:12px;color:#666}
			.ht-snip-page{background:#fff;border:1px solid #e0a800;color:#8a5a00;border-radius:7px;padding:4px 11px;cursor:pointer;font-size:12px}
			.ht-snip-page[disabled]{opacity:.4;cursor:default}
			...
			(function(){
				...
				var I18N = {
					...
					nomatch: <?php echo wp_json_encode( __( 'No snippet matches your search.', 'horse-tools' ) ); ?>,
					allTags: <?php echo wp_json_encode( __( 'All snippets', 'horse-tools' ) ); ?>,
					/* translators: 1: snippets on this page, 2: snippets in total. */
					showing: <?php echo wp_json_encode( __( '%1$d of %2$d', 'horse-tools' ) ); ?>,
					/* translators: 1: current page, 2: number of pages. */
					pageOf: <?php echo wp_json_encode( __( 'Page %1$d of %2$d', 'horse-tools' ) ); ?>,
					gone: <?php echo wp_json_encode( __( 'That snippet no longer exists.', 'horse-tools' ) ); ?>
				};
				var COND = {
				...
				// The first page only. Searching, filtering and paging are the
				// server's job — the screen never holds the whole site's
				// snippets, and never holds a single snippet's body until the
				// moment somebody opens it to edit.
				var DATA = <?php echo wp_json_encode( horsetools_snip_list_payload() ); ?>;
				var pager=$('ht-snip-pager'), pagelbl=$('ht-snip-pagelbl');
				...
				function refreshTagFilter(){
					...
					var html='<option value="">'+esc(I18N.allTags)+'</option>';
					(DATA.tags||[]).forEach(function(t){
						html+='<option value="'+esc(t.slug)+'">'+esc(t.name)+' ('+t.count+')</option>';
					});
				...
				function render(){
					...
					var rows=DATA.items||[];
					if (countlbl){
						countlbl.textContent = DATA.total
							? I18N.showing.replace('%1$d',rows.length).replace('%2$d',DATA.total)
							: '';
					}
					if (pager){
						pager.hidden = DATA.pages<2;
						if (pagelbl){ pagelbl.textContent=I18N.pageOf.replace('%1$d',DATA.page).replace('%2$d',DATA.pages); }
						pager.querySelector('[data-step="-1"]').disabled = DATA.page<=1;
						pager.querySelector('[data-step="1"]').disabled  = DATA.page>=DATA.pages;
					}
					if (!rows.length){
						var searching = (search&&search.value.trim()) || tagFilter.value;
						list.innerHTML = '<div class="ht-snip-msg">'+esc(searching?I18N.nomatch:I18N.none)+'</div>';
						return;
					}
					var html='';
				...
				// Whatever the list is currently showing, so a save or a delete
				// lands the user back where they were rather than on page one.
				function listArgs(page){
					return { s:(search&&search.value||'').trim(), tag:tagFilter.value,
						page: page || DATA.page || 1 };
				}
				function reload(page){
					var a=listArgs(page); a.action='horsetools_snip_page';
					post(a, function(res){
						if (res&&res.success){ DATA=res.data; render(); }
						else { say(msg,I18N.fail,'err'); }
					});
				}
				// A search or a tag change is a new query, so it starts at page one.
				var searchTimer=null;
				tagFilter.addEventListener('change', function(){ reload(1); });
				if (search){ search.addEventListener('input', function(){
					clearTimeout(searchTimer); searchTimer=setTimeout(function(){ reload(1); },250);
				}); }
				if (pager){ pager.addEventListener('click', function(e){
					var b=e.target.closest('[data-step]'); if(!b||b.disabled){ return; }
					reload(Math.max(1, DATA.page + parseInt(b.dataset.step,10)));
				}); }
				...
				list.addEventListener('click', function(e){
					...
					// The body is fetched here, for this one snippet, rather than
					// having been sitting in the page since it loaded.
					if (ed){ post({action:'horsetools_snip_get',slug:ed.dataset.edit}, function(res){
						if (!res||!res.success){ say(msg,(res&&res.data&&res.data.msg)||I18N.gone,'err'); return; }
						var s=res.data;
						...
						if (F.phpHook){ F.phpHook.value=s.hook||''; }
						if (F.phpScope){ F.phpScope.value=s.scope||'front'; }
						...
						F.name.focus(); window.scrollTo({top:0,behavior:'smooth'});
					}); return; }
					if (dl){ if(!confirm(I18N.confirmDel))return; var d=listArgs(); d.action='horsetools_snip_delete'; d.slug=dl.dataset.del;
						post(d, function(res){ if(res&&res.success){ DATA=res.data.list; render(); say(msg,I18N.deleted,'good'); } else { say(msg,(res&&res.data&&res.data.msg)||I18N.fail,'err'); } }); return; }
				});
				...
				$('ht-snip-save').addEventListener('click', function(){
					htFlushEditor();
					var d=listArgs();
					d.action='horsetools_snip_save'; d.slug=F.name.value; d.title=F.title.value; d.desc=F.desc.value;
					d.content=F.content.value; d.on=F.on.value; d.no_admin=F.noadmin.value; d.device=F.device.value;
					d.login=F.login.value; d.role=F.role.value; d.tags=F.tags.value;
					d.date_from=F.from.value; d.date_to=F.to.value;
					d.php=(F.php&&F.php.checked)?'1':'0';
					d.php_hook=F.phpHook?F.phpHook.value:''; d.php_scope=F.phpScope?F.phpScope.value:'front';
					post(d, function(res){
						if (res&&res.success){ DATA=res.data.list; render(); say(msg,I18N.saved,'good'); }
						else { say(msg,(res&&res.data&&res.data.msg)||I18N.fail,'err'); }
					});
				});
				...
				$('ht-snip-import').addEventListener('click', function(){
					...
					post(d, function(res){
						if (res&&res.success){ DATA=res.data.list; render(); say(msg,I18N.imported.replace('%1$d',res.data.imported).replace('%2$d',res.data.skipped),'good'); }
						else { say(msg,(res&&res.data&&res.data.msg)||I18N.fail,'err'); }
					});
				});
