...
define( 'HORSETOOLS_VERSION', '1.3.42' );
...
if ( is_admin() ) {
	...
	include( HORSETOOLS_DIR . 'inc/mail-presets.php' );
...
// admin page loads, and WP-Cron is not is_admin(). Gating these two on is_admin()
// would register the cron hook only in requests where cron never runs — which is
// exactly how this plugin once shipped a scheduled task that never fired once.
if ( horsetools_is_backend() ) {
	include( HORSETOOLS_DIR . 'inc/alert.php' );
	include( HORSETOOLS_DIR . 'inc/watch-heartbeat.php' );
}
